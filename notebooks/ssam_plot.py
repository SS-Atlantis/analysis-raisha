...
def plot_pah(pah, scenario1, scenario2, scenario3, time, start, end, event_start): 
    ...
    fig, ax = plt.subplots(figsize = (14,3))
    ax.plot(time[start:end], Phe1[start:end], time[start:end], Phe2[start:end], time[start:end], Phe3[start:end])
    ax.plot(event_start, 0, 'ro', alpha=0.5)
    ax.set_title('Concentration of ' + pah + ' in Salish Sea Atlantis', fontsize = font_size)
    ax.set_ylabel('mg Phe m$^{-3}$')
    ax.legend(['scenario 1', 'scenario 2', 'scenario 3'], loc='best');
...
def plot_pelagic_pah_in_bio(group, scenario1, scenario2, scenario3, pah, time, start, end, event_start):
    ...
    for species in group:
        ...
        fig, ax = plt.subplots(figsize = (14,3))
        ax.plot(time[start:end], p_oiled1, time[start:end], p_oiled2, time[start:end], p_oiled3, linewidth = 2)
...
def boxplot_pelagic(group, scenario, control, days, data_labels, x_lim=None): #bacteria, plankton
    ...
    df1.plot(kind="barh", subplots=True, layout=(1,3), figsize=(15,4), sharey=True, sharex=True, color=bio_colours, legend=None,
...
def boxplot_benthic(group, scenario, control, days, data_labels, x_lim=None): 
...
def benthic_compare_scenarios(group, scenario1, scenario2, scenario3, control, time, start, end, y_min=None, y_max=None): # benthos, shellfish
    ...
    for species in group:
        ...
        ax.plot(time[start:end], control_ratio, 'k', time[start:end], ratio1, time[start:end], ratio2, time[start:end], ratio3, linewidth = 2)
        ax.set_ylim([y_min, y_max])
        plt.ylabel('Percent (%) change', fontsize=12)
        ax.legend(['control', 'scenario 1', 'scenario 2', 'scenario 3'])
        ax.set_title(species);

def pelagic_compare_scenarios(group, scenario1, scenario2, scenario3, control, time, start, end, y_min=None, y_max=None): #bacteria, plankton, sharks, birds, mammals, named_fish, salmon, fish, benth_feeders
...
def sediment_compare_scenarios(group, scenario1, scenario2, scenario3, control, time, start, end, y_min=None, y_max=None): # benthos, shellfish
